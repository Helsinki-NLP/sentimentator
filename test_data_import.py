...
from sentimentator.model import db, Language, TestSentence, User
...
def import_test_data(lang, lang_data):
    with app.app_context():
        language_id = ensure_language(lang)
        for i in lang_data:
            db.session.add(TestSentence(
...
def main():
    ...
    if args.LANG == 'en': # In this case the pivot language is English
        ...
        import_test_data(args.LANG, lang_data)
